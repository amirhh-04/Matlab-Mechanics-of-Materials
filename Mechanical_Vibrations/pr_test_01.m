...
y_front = @(t) (t >= start_time & t < start_time+a/(2*v)) .* (2*c/a) .* (v*t - start_time*v) + ...
               (t >= start_time+a/(2*v) & t < start_time+(a-b_bump)/(2*v)) .* c + ...
               (t >= start_time+(a-b_bump)/(2*v) & t < start_time+a/(2*v)) .* (2*c/a) .* (start_time*v - v*t + a/2) + ...
               (t >= start_time+a/(2*v) & t < start_time+a/v) .* (-2*c/a) .* (v*t - start_time*v - a/2) + ...
               (t >= start_time+a/v) * 0;

y_rear = @(t) (t >= start_time+d/v & t < start_time+(a+d)/(2*v)) .* (2*c/a) .* (v*(t - start_time - d/v)) + ...
               (t >= start_time+(a+d)/(2*v) & t < start_time+(a-b_bump+d)/(2*v)) .* c + ...
               (t >= start_time+(a-b_bump+d)/(2*v) & t < start_time+(a+d)/(2*v)) .* (2*c/a) .* ((start_time + d/v)*v - v*t + a/2) + ...
               (t >= start_time+(a+d)/(2*v) & t < start_time+(a+d)/v) .* (-2*c/a) .* (v*(t - start_time - d/v) - a/2) + ...
               (t >= start_time+(a+d)/v) * 0;
